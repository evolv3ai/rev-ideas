...
def analyze_code_changes(diff: str, changed_files: List[str], pr_info: Dict[str, Any]) -> str:
...
Keep your response concise but thorough. Focus on actionable feedback."""
    ...
    try:
        ...
        result = subprocess.run(
            ['gemini', '-m', 'gemini-2.5-pro'],  # Use the pro model for better code review
            input=prompt,
            capture_output=True,
            text=True,
            check=True
...
def post_pr_comment(comment: str, pr_info: Dict[str, Any]):
    ...
    try:
        ...
        subprocess.run([
            'gh', 'pr', 'comment', pr_info['number'],
            '--body-file', '/tmp/gemini_comment.md'
        ], check=True)
...
def main():
    ...
    if not check_gemini_cli():
        ...
        print("3. Run 'gemini' command to authenticate (happens automatically on first use)")
